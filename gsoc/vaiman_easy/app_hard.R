...
#' }
#'
explorapp_hard <- function(ui, server) {
  X1 <- NULL
  X2 <- NULL
  ppred <- NULL
  Sim <- NULL
  ...
  ppbound <- function(ru, data, test, meth, entro, title, simM = FALSE) {
    grilla <-
      base::expand.grid(
        X1 = seq(
          (min(data$X1) + sign(min(data$X1)) * .5),
          (max(data$X1) + sign(max(data$X1)) * .5),
          length.out = 100
        ),
        X2 = seq(
          (min(data$X2) + sign(min(data$X2)) * .5),
          (max(data$X2) + sign(max(data$X2)) * .5),
          length.out = 100
        )
      )
    ...
    if (meth == "Rpart") {
      ...
      err <-
        round(
          1 -
            sum(diag(table(
              predict(rpart.mod, newdata = test[, -1], type = "class"),
              test[, 1]
            ))) /
              nrow(test[, -1]),
          3
    ...
    if (meth == "Modified") {
      ...
      pptree <- PPtreeExt::PPtreeExt_split(
        Sim ~ .,
        data = data,
        "LDA",
        entro = entro
    ...
    function(
      ...
      if (simM) {
        pl.pp <-
          ggplot2::ggplot(data = grilla) +
          ggplot2::geom_point(
            ggplot2::aes(x = X1, y = X2, color = ppred),
            alpha = .20
          ) +
          ggplot2::scale_colour_brewer(
            name = "Class",
    ...
    shiny::tabsetPanel(
        ...
        shiny::fluidRow(
          ...
          shiny::column(
            ...
            shiny::selectInput(
              inputId = "modi2",
              label = "Modification",
              choices = c("Subsetting clases" = "1", "Multiple splits" = "3"),
              selected = 3
            )
          )
        ),
      ...
      shiny::tabPanel(
        ...
        shiny::fluidRow(
          ...
          shiny::column(
            ...
            shiny::numericInput(
              "BarOmega",
              label = "BarOmega desired average overlap",
              value = 0.05
            )
          )
        ),
        shiny::fluidRow(
          shiny::column(
            ...
            shiny::numericInput(
              "MaxOmega",
              label = "MaxOmega desired maximum overlap",
              value = 0.15
            )
          ),
          shiny::column(
      ...
      shiny::tabPanel(
        ...
        shiny::fluidRow(
          shiny::column(
            ...
            shiny::radioButtons(
              "drawClass",
              "Select class:",
              choices = c("sim1", "sim2", "sim3"),
              inline = TRUE
            )
          ),
          shiny::column(
            2,
            shiny::selectInput("drawRule", "Rule", choices = 1:8, selected = 1)
          ),
          shiny::column(
            3,
            shiny::selectInput(
              "drawModi",
              "Modification",
              choices = c("Subsetting" = "1", "Multiple splits" = "3"),
              selected = 3
            )
          )
        ),
        ...
        shiny::fluidRow(
          ...
          shiny::uiOutput("drawBoundariesUi")
        )
      )
    )
  ))
  ...
  server <- function(input, output) {
    ...
    output$distPlot2 <- shiny::renderPlot({
      if (input$do2) {
        ...
        aux2 <-
          data.frame(
            Sim = rep(
              paste("sim", as.numeric(input$group), sep = ""),
              round(x6 * 0.25)
            ),
    ...
    drawn_data <- shiny::reactiveVal(data.frame(
      Sim = character(),
      X1 = numeric(),
      X2 = numeric()
    ))
    ...
    shiny::observeEvent(input$canvas_click, {
      ...
      if (!is.null(click)) {
        drawn_data(rbind(
          drawn_data(),
          data.frame(Sim = input$drawClass, X1 = click$x, X2 = click$y)
        ))
      }
    })
    ...
    output$drawCount <- shiny::renderText({
      paste("Points:", nrow(drawn_data()))
    })
    ...
    output$drawBoundaries <- shiny::renderPlot({
      ...
      if (nrow(d) < 5 || length(unique(d$Sim)) < 2) {
        return(plot(0, main = "Need 5 points & 2+ classes"))
      }
      ...
      if (shiny::isolate(input$drawModi) == "1") {
        modpl <- ppbound(
          as.numeric(shiny::isolate(input$drawRule)),
          data = d,
          test = d,
          meth = "Modified",
          entro = FALSE,
        ...
        modpl <- ppboundMOD(
          ...
          entroindiv = TRUE,
          title = "PPtreeExt: Mul",
          strule = as.numeric(shiny::isolate(input$drawRule)),
          tot = nrow(d)
        )
      }
